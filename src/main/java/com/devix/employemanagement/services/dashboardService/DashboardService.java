...
public class DashboardService implements IDashboardService {
    ...
    public DashboardResponseDto getDashboardRedis(Long employeeId) {
        log.info("Fetching dashboard for employeeId: {}", employeeId);
        Employee employee = employeeRepository.findById(employeeId).orElseThrow(() -> new RuntimeException("Employee not found"));
        Long orgId = employee.getOrganization().getId();
        Long officeId = employee.getPrimaryOffice().getId();
        AttendanceDaily attendance = attendanceRepository
    ...
    @Override
    public DashboardResponseDto getDashboardRedisDeleted(Long employeeId) {
        log.info("Delete cache for employeeId: {}", employeeId);
        Employee employee = employeeRepository.findById(employeeId).orElseThrow(() -> new RuntimeException("Employee not found"));
